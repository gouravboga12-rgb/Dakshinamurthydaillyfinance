        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-md p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 print:border-none print:shadow-none">
          ...
          <div className="flex justify-between items-start border-b-2 border-slate-100 pb-5">
            ...
            <div className="text-right text-xs text-slate-400">
              <div>Date: <span className="font-bold text-slate-700">{reportData.date || new Date(Date.now() + 5.5*60*60*1000).toISOString().split('T')[0]}</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 max-w-md">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-1">
                  ...
                  <span className="text-2xl font-extrabold text-slate-900">
                    ₹{(reportData.data?.reduce((sum: number, item: any) => sum + (item.loan?.daily_installment ?? item.daily_installment ?? 200), 0) || 0).toLocaleString('en-IN')}
                  </span>
                  <span className="text-[9px] text-slate-400 block">All installments marked paid today (IST)</span>
                </div>
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-1">
                  ...
                  <span className="text-[9px] text-slate-400 block">Includes catch-up payments</span>
                </div>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-200 text-xs font-bold text-slate-400 uppercase tracking-wider">
                      ...
                      <th className="py-3 px-2">Installment Due Date</th>
                      <th className="py-3 px-2">Collected At (IST)</th>
                  ...
                  <tbody className="divide-y divide-slate-100 text-sm text-slate-700 font-medium">
                    {reportData.data?.length === 0 ? (
                      ...
                      (() => {
                        // Compute today in IST for catch-up detection
                        const todayIST = new Date(Date.now() + 5.5 * 60 * 60 * 1000).toISOString().split('T')[0];
                        return (reportData.data || []).map((item: any) => {
                          const isCatchup = item.due_date && item.due_date < todayIST;
                          const collectedAt = item.payment_date ? new Date(item.payment_date).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'short', timeStyle: 'short' }) : '—';
                          return (
                            <tr key={item.id} className={isCatchup ? 'bg-amber-50/50' : ''}>
                              <td className="py-3.5 px-2 font-bold text-slate-800">
                                {item.loan?.customer?.full_name || 'Unknown'}
                                {isCatchup && (
                                  <span className="ml-2 px-1.5 py-0.5 rounded text-[9px] font-bold bg-amber-100 text-amber-700 border border-amber-200">Catch-up</span>
                                )}
                              </td>
                              <td className="py-3.5 px-2 font-mono text-xs text-slate-500">{item.loan?.customer?.mobile_number || 'N/A'}</td>
                              <td className="py-3.5 px-2 font-mono text-xs text-slate-600 font-semibold">{item.due_date || '—'}</td>
                              <td className="py-3.5 px-2 font-mono text-xs text-slate-500">{collectedAt}</td>
